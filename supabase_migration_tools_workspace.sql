...
create table if not exists public.tools_workspace_accounts (id uuid primary key default gen_random_uuid(),name text not null,username text not null unique,salt text not null,password_hash text not null,can_issue boolean not null default false,created_at timestamptz not null default now());
...
alter table public.tools_workspace_accounts enable row level security; alter table public.tools_workspace_sessions enable row level security; alter table public.tools_workspace_employees enable row level security; alter table public.tools_workspace_equipment enable row level security; alter table public.tools_workspace_history enable row level security; alter table public.tools_workspace_changes enable row level security; alter table public.tools_workspace_idempotency enable row level security; alter table public.tools_workspace_evidence enable row level security; alter table public.tools_workspace_usage enable row level security; alter table public.tools_workspace_errors enable row level security; alter table public.tools_workspace_feedback enable row level security;
