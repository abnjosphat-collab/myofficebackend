...
create table if not exists public.tools_workspace_source_registers (id uuid primary key default gen_random_uuid(),department text not null,notes text,storage_path text not null unique,original_name text not null,content_type text not null,size_bytes bigint not null,uploaded_by_account_id uuid references public.tools_workspace_accounts(id),uploaded_by text not null,uploaded_at timestamptz not null default now());
...
create index if not exists tools_workspace_source_registers_uploaded_idx on public.tools_workspace_source_registers(uploaded_at desc);
...
alter table public.tools_workspace_accounts enable row level security; alter table public.tools_workspace_sessions enable row level security; alter table public.tools_workspace_employees enable row level security; alter table public.tools_workspace_equipment enable row level security; alter table public.tools_workspace_history enable row level security; alter table public.tools_workspace_changes enable row level security; alter table public.tools_workspace_idempotency enable row level security; alter table public.tools_workspace_evidence enable row level security; alter table public.tools_workspace_source_registers enable row level security; alter table public.tools_workspace_usage enable row level security; alter table public.tools_workspace_errors enable row level security; alter table public.tools_workspace_feedback enable row level security; alter table public.tools_workspace_notification_reads enable row level security;
