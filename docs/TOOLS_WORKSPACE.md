...
- Signed-in viewers can read tools, employees, history, notifications and analytics.
- Administrator accounts are required to create or modify register records, change custody, import data, attach evidence, archive/restore, and undo/redo.
...
Run `python -m pytest tests/test_tools_workspace_router.py -q` after changing this contract. Tests cover role boundaries, custody history, durable undo/redo, derived overdue state, account-scoped notification acknowledgement, analytics, import and attachment metadata.
